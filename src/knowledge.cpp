...
void WorldSet::union_with(const WorldSet &other, std::size_t active_word_count) noexcept {
    for (std::size_t i = 0; i < active_word_count; i++) {
        words[i] |= other.words[i];
    }
}
...
std::pair<bool, int> can_guess(const KnowledgeState &ks, PlayerId v) {
    ...
    if (candidates.empty(ks.active_word_count))
        return {false, -1};
    ...
    return seen_any ? std::pair<bool, int>{true, decided_color} : std::pair<bool, int>{false, -1};
}
